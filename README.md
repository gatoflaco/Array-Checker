...
- There may be nothing before not after the array. It should hold that the array has as at least one row, and every row should have an equal number of columns. The columns in a given row of the array should be separated by whitespace, and all rows should be on their own lines.
